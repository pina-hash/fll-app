...
- **`anon` holds no table grant anywhere and can execute exactly FIVE functions**: `team_login_roster`, `student_self_enroll`, `team_size_cap` (0013), `parent_view` and `parent_photo_path` (0014). Each is a door the spec asked for, and the list in `tests/schema-catalog.test.ts` is what makes a sixth a decision rather than an accident. Every one of them is SECURITY DEFINER, builds any address or path it needs rather than accepting one, and answers a caller it does not like with NULL rather than an error.
- **SIX ACTIVE STUDENTS PER TEAM, AND THE RULE IS A TRIGGER.** `team_size_cap()` is the number; `_students_team_cap` (0013) is a BEFORE INSERT OR UPDATE trigger that takes `pg_advisory_xact_lock` on the team and then counts. It is a trigger and not a CHECK because it counts rows, and it locks because twenty children enrolling from twenty phones would each otherwise read five. A deactivated row holds no seat. ANY new path that puts a student on a team is covered by it without saying so; do not re-implement the count anywhere except as a friendlier pre-check inside an RPC.
...
- **Students:** no real email. Two doors, both minting the same three rows behind the same `fll.creating_student` flag: `student_create` (mentor-only RPC) writes `auth.users` + `auth.identities` + `students` in one transaction, raising a transaction-local flag (`fll.creating_student`) that the trigger requires for any `@fll.invalid` address. Address = `{join_code lowercased}-{slug}@fll.invalid`; `.invalid` is RFC 2606 reserved. Slug = lowercased first name + last initial in `[a-z0-9]`, deduped per team with a numeric suffix, STORED so a rename never changes a login. PIN = 6 digits (GoTrue's 6-character minimum). The second door is `student_self_enroll` (0013), granted to `anon`, because the roster is built live in the room on the first Friday: a child with the team code, an open join window and a free seat types their own name and PIN and is signed in immediately. There is NO approval queue, deliberately -- a queue means twenty children waiting on one adult.
- **A JOIN WINDOW IS DERIVED, NOT A FLAG SOMEBODY CLEARS.** `team_join_open(team_id)` (0013) is the one statement: a mentor opened it, the meeting it was opened in has not ended, and the local day it was opened on has not. `meeting_end` also clears the stored columns so the console is honest, but every gate calls the function. Never read `teams.join_open_since` to decide anything.
