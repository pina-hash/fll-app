...
- **THE HOSTED PROJECT CARRIES DEFAULT PRIVILEGES THE LOCAL IMAGE DOES NOT, SO A GRANT VERIFIED ONLY AGAINST LOCAL PROVES NOTHING.** The line above describes the LOCAL stack. A hosted Supabase project ships with `alter default privileges for role postgres in schema public grant ... to anon, authenticated, service_role` for functions, tables and sequences. Migrations connect as `postgres`, so on the linked project every `create function` in this chain arrived already executable by `anon` and `authenticated`, and every `create table` arrives with full DML for both. Nothing in `supabase/` asks for this and nothing in `supabase/` can see it.
