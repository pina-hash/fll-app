 ...
 * touches nothing.
 */
import { launch, openPage, settle, waitUntil, clickUntil, selectUntil } from './browser.mjs';
import {
	horizontalScroll,
	contrast,
	tapTargets,
...
const CASES = [
	...
	{
		/*
			ADDED IN THIS REPO'S PORT, with `selectUntil`. A `<select>` cannot be
			coordinate-clicked -- the click opens a native popup no predicate can
			see -- so the harness needs its own select step, and a step nobody has
			ever seen fail is a step nobody has tested.

			THE FIXTURE REPRODUCES THE REAL FAILURE, WHICH IS TIMING. On
			/dev/route-planner a one-shot `evaluate` set the select's value,
			dispatched `change`, reported success, left the control READING the
			chosen option, and never reached Svelte's binding: the binding was not
			attached yet. Retried, the same selection took four attempts. So the
			bad fixture attaches its handler 900ms late, which no single attempt
			can catch and the retry loop can; the good one is the same page and the
			same handler with the retries the step actually performs.
		*/
		group: 'prepare-select (a binding that is not attached yet)',
		bad: {
			name: 'one attempt, against a handler attached 900ms after load',
			html: shell(
				'<select id="s"><option value="a">a</option><option value="b">b</option></select><div id="out">at rest</div>' +
					'<script>setTimeout(() => { document.getElementById("s").addEventListener("change", (e) => { document.getElementById("out").className = "done"; }); }, 900)</' +
					'script>'
			),
			run: async (p) => {
				const step = { select: '#s', value: 'b', until: '() => !!document.querySelector("#out.done")' };
				const r = await selectUntil(p, step.select, step.value, step.until, { attempts: 1, gapMs: 100 });
				return prepareSelectResult(step, r);
			},
			expect: 'outside'
		},
		good: {
			name: 'the same page and handler, with the step\'s own retries',
			html: shell(
				'<select id="s"><option value="a">a</option><option value="b">b</option></select><div id="out">at rest</div>' +
					'<script>setTimeout(() => { document.getElementById("s").addEventListener("change", (e) => { document.getElementById("out").className = "done"; }); }, 900)</' +
					'script>'
			...
			run: async (p) => {
				const step = { select: '#s', value: 'b', until: '() => !!document.querySelector("#out.done")' };
				const r = await selectUntil(p, step.select, step.value, step.until, { attempts: 12, gapMs: 300 });
				return prepareSelectResult(step, r);
			},
			expect: 'within'
		}
	...
	{
		/* The pre-selection short-circuit, which is `prepare-click`'s trap one
		   control over: a predicate the page satisfies AT REST means nothing was
		   selected and the step reached no state, while the report says
		   "selected". The two fixtures differ only in the predicate. */
		group: 'prepare-select (a predicate the page satisfies at REST)',
		bad: {
			name: 'the `until` already holds, so nothing is selected',
			html: shell(
				'<select id="s" onchange="document.getElementById(\'out\').className=\'done\'"><option value="a">a</option><option value="b">b</option></select><div id="out">at rest</div>'
			),
			run: async (p) => {
				const step = { select: '#s', value: 'b', until: '() => !!document.querySelector("#out")' };
				const r = await selectUntil(p, step.select, step.value, step.until, { attempts: 3, gapMs: 80 });
				return prepareSelectResult(step, r);
			},
			expect: 'outside'
		},
		good: {
			name: 'the same selection under a predicate only the selection can satisfy',
			html: shell(
				'<select id="s" onchange="document.getElementById(\'out\').className=\'done\'"><option value="a">a</option><option value="b">b</option></select><div id="out">at rest</div>'
			),
			run: async (p) => {
