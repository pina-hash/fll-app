 ...
 * written to a .env file: SvelteKit merges process.env into
 * $env/static/public, so the dev server boots with no file created and
 * nothing to restore. The values are placeholders and no dev route reaches
 * Supabase with them -- every route under src/routes/dev/ mounts a real
 * component with fixture props and calls no client.
 *
 * Ported from idea-app's tools/browser-verify/server.mjs. The one changed
 * line is the route the boot probe hits, which has to exist in THIS repo.
 */
...
/** The route the boot probe asks for. It must be a real dev route here. */
const PROBE_PATH = '/dev/route-planner';
...
export async function startDevServer({ port = 5199, host = '127.0.0.1', cwd, bootTimeoutMs = 180_000, quiet = true } = {}) {
	...
	while (Date.now() - started < bootTimeoutMs) {
		...
		const status = await probe(`${origin}${PROBE_PATH}`, { timeoutMs: 5000 });
		if (status !== null) {
			return {
				origin,
				port,
