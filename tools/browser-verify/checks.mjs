...
/**
 * A `prepare` SELECT step, judged the way a click step is.
 *
 * ADDED IN THIS REPO'S PORT alongside `selectUntil` (see browser.mjs). It
 * keeps the click step's two gates for the same reasons: the selection must
 * have ACTUALLY HAPPENED (`attempts >= 1`, so a predicate the page satisfies
 * at rest is a finding rather than a silent no-op), and a predicate must
 * exist, because a selection nothing verified is a step that reports success
 * having checked nothing.
 */
export function prepareSelectResult(step, r) {
	const hasPredicate = !!step.until;
	const skipped = r.ok && r.attempts === 0;
	const ok = r.ok && r.attempts > 0 && hasPredicate;
	const why = !r.ok
		? r.reason
		: skipped
			? 'the predicate ALREADY HELD, so nothing was selected -- this step reached no state'
			: !hasPredicate
				? 'selected, but with no `until` the effect was never verified'
				: r.reason;
	return {
		check: 'prepare-select',
		label: `${step.select} = ${JSON.stringify(step.value)}`,
		measured: `${r.matched} matched, ${r.attempts} attempt(s), ${why}`,
		threshold: 'the option is selected at least once and its `until` then holds',
		withinThreshold: ok,
		data: { ...r, hasPredicate }
	};
}
