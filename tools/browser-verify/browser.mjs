...
/**
 * Choose an option in a `<select>` and keep trying until it demonstrably took.
 *
 * ADDED IN THIS REPO'S PORT; idea-app's harness has no select step. The reason
 * is `/dev/route-planner`, whose every interesting state (which picture, which
 * scenario, whether the mat is set up) is behind a `<select>` in the harness
 * bar rather than behind a button.
 *
 * A SELECT CANNOT BE COORDINATE-CLICKED. `clickUntil` dispatches a real mouse
 * click at the element's centre, which on a `<select>` opens the browser's
 * native popup -- a widget outside the page that no page-side predicate can
 * see and no second click can reach. So this uses Playwright's
 * `selectOption`, which sets the value and fires `input` and `change` the way
 * the browser does.
 *
 * IT RETRIES FOR THE REASON `clickUntil` RETRIES, AND THE COST OF NOT DOING SO
 * WAS MEASURED HERE. Paint is not interactivity: a one-shot `evaluate` that
 * set `.value` and dispatched `change` reported success, left the select
 * READING "calibrated", and never reached Svelte's binding, because the
 * binding was not attached yet. The picture never rendered and three
 * measurements at each width came back as findings about a surface the run had
 * not reached. Retried, the same selection took FOUR attempts (~1.2s after
 * `settle`) before the binding was live. The attempt count is returned and
 * printed for that reason: a step that needed four tries is telling you
 * something a silent success would not.
 */
export async function selectUntil(page, selector, value, until, { attempts = 12, gapMs = 300 } = {}) {
	const matched = await page.locator(selector).count();
	if (matched === 0) return { ok: false, matched: 0, attempts: 0, reason: 'no match' };
	...
	/* Same pre-check, and the same trap, as `clickUntil`: a predicate the page
	   satisfies at REST means the selection never happened and the step
	   reached no state. `prepareSelectResult` reports that as a finding. */
	...
	for (let i = 1; i <= attempts; i++) {
		...
		await page.waitForTimeout(gapMs);
		const s = await satisfied();
		if (s === null) return { ok: true, matched, attempts: i, reason: 'selected (no predicate given)' };
		if (s === true) return { ok: true, matched, attempts: i, reason: 'predicate satisfied' };
	}
	return { ok: false, matched, attempts, reason: 'predicate never satisfied' };
}
