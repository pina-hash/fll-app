 ...
 *   npm run verify:browser -- --route route-planner
...
import { launch, openPage, settle, waitForApp, clickUntil, selectUntil, waitUntil } from './browser.mjs';
...
export const BREAKAGE = {
	...
	overflow:
		'.harness, main, .surface, .surface__body, .rp, .shell, body > div { min-width: 1600px !important; }',
	...
	invisible:
		'.harness, main, h1, table, .rp, .mat, .card, .surface__body { opacity: 0 !important; }',
	...
	'blank-text': { js: 'for (const el of document.querySelectorAll(".bf__tm, footer p")) el.textContent = "";' },
	...
	motion: {
		css:
			...
			'.mark svg *, .mark img, .live-dot { animation: bv-break-spin 2s linear infinite !important; }'
	},
...
async function runRoute(browser, origin, spec, width, opts) {
	...
	try {
		...
		for (const step of spec.prepare ?? []) {
			...
			if (step.select) {
				/* A `<select>` cannot be coordinate-clicked: the click opens a
				   native popup no page-side predicate can see and no second click
				   can reach. `selectUntil` uses Playwright's selectOption and
				   RETRIES, because the binding behind it is not attached the moment
				   the page paints -- browser.mjs records the four-attempt
				   measurement that put this step here. */
				const r = await selectUntil(page, step.select, step.value, step.until, {
					attempts: step.attempts ?? 12,
					gapMs: step.gapMs ?? 300
				});
				results.push(prepareSelectResult(step, r));
			}
			if (step.waitFor) {
...
async function main() {
	...
	if (!specs.length) {
		console.error('No routes matched. Known routes are the files under tools/browser-verify/routes/.');
		return 2;
	}
